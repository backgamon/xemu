...
unsigned int pgraph_gl_bind_inline_array(NV2AState *d)
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, r->gl_inline_array_buffer);
//    GLsizeiptr buffer_size = quantize_buffer_alloc(index_count * vertex_size);
//    glBufferData(GL_ARRAY_BUFFER, buffer_size, NULL, GL_STREAM_DRAW);

    assert(index_count * vertex_size <= NV2A_MAX_BATCH_LENGTH_V2 * sizeof(uint32_t) && "This title uses inline arrays that are too large for this test");
    glBufferData(GL_ARRAY_BUFFER, NV2A_MAX_BATCH_LENGTH_V2 * sizeof(uint32_t), NULL, GL_STREAM_DRAW);
