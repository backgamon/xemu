...
static void update_shader_constant_locations(ShaderBinding *binding)
{
    ...
    binding->uniform_locs.vsh.color_key =
        uniform_index(&binding->fragment->uniforms, "colorKey");

    binding->uniform_locs.vsh.color_key_mask =
...
static void shader_update_constants(PGRAPHState *pg, ShaderBinding *binding)
{
    ...
    if (binding->uniform_locs.vsh.color_key != -1) {
        ...
        uniform1uiv(&binding->fragment->uniforms,
                    binding->uniform_locs.vsh.color_key, 4, color_key_colors);
    }
    ...
    if (binding->uniform_locs.vsh.color_key_mask != -1) {
        uniform1uiv(&binding->fragment->uniforms,
                    binding->uniform_locs.vsh.color_key_mask, 4,
                    color_key_mask);
    }
